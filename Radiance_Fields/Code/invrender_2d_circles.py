# Inverse renderer of 2D images using point-based primitives: circles
#
...
def circle_blob(center, size_x, size_y, opacity, color, x, y):
    dx = (x - center[0]) / size_x
    dy = (y - center[1]) / size_y
    distance_squared = dx**2 + dy**2
    mask = (dx**2 + dy**2) <= 1
    return opacity * mask[:, :, None] * color
